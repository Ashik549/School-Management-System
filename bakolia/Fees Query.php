...
<?php
    ...
    //global $fetch;
    $fees_Qu=mysql_query("SELECT * FROM `fees query`") or die("error");
    echo '<table style="width:80%;">';
    echo '<th align="left">STUDENT ID</th><th align="left">FEES</th><th align="left" >STATUS</th>';
    if($fees_Qu)
    {
        while ($feesr= mysql_fetch_assoc($fees_Qu))
        {
            echo '<tr><td>'.$feesr['ID'].'</td>'
               . '<td>'.$feesr['Fees'].'</td>'
               . '<td>'.$feesr['Status'].'</td></tr>';
