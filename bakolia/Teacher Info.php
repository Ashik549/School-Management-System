...
<?php
    ...
    global $fetch;
    $result=mysql_query("select * from teacher") or die("error");
    echo '<table style="width:80%;">';
    echo '<th align="left">NAMES</th><th align="left">ID</th><th align="left" >DESIGNATION</th><th align="left">SALARY</th><th align="left">STATUS</th><th align="left" colspan=2>ACTION</th>';
    if($result)
    {
        
        while ($fetch= mysql_fetch_assoc($result))
        {
            if($_SESSION ['user_id']==1997001)
            {
                if($fetch['Designation']=='Head Master')
                {
                    echo '<tr><td>'.$fetch['Name'].'</td>'
                    . '<td>'.$fetch['ID'].'</td>'
                    . '<td>'.$fetch['Designation'].'</td>'
                    . '<td>'.$fetch['Salary'].'</td></tr>';
                }
                else
                {
                    echo '<tr><td>'.$fetch['Name'].'</td>'
                    ...
                    . '<td>'.$fetch['Designation'].'</td>'
                    . '<td>'.$fetch['Salary'].'</td>'
                    .'<td><a href=update_student.php?id='.$fetch['ID'].'><img src="Edit.png" width="30" height="30" title=MODIFY_RECORD /></a></td>'
                    . '<td><a href=deletetr.php?id='.$fetch['ID'].'>'
                    . '<img src="deletee.png" width="30" height="30" title=DELETE_RECORD /></a></td></tr>';
                }
            }
            else {
                if($fetch['Designation']=="Senior"||$fetch['Designation']=='Head Master')
                {
                    echo '<tr><td>'.$fetch['Name'].'</td>'
                    . '<td>'.$fetch['ID'].'</td>'
                    . '<td>'.$fetch['Designation'].'</td>'
            ...
            /*else if($fetch['Designation']=='Senior')
            {
